...
export const FormItemLayout: FC<FormItemLayoutProps> = ({ title, description, disabled, required, children, layout = 'row' }) => {
  return (
    <FormItem className={cn(
      'space-y-0 flex',
      { 'flex-col justify-start gap-2': layout === 'col' },
      { 'justify-between items-center': layout === 'row' }
    )}>
      <div>
...
export const SubmitButton: FC<{ disabled: boolean }> = ({ disabled }) => {
  return (
    <Button type='submit' className='flex gap-2 items-center' disabled={disabled}>
